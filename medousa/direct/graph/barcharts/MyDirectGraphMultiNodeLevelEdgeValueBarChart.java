...
public class MyDirectGraphMultiNodeLevelEdgeValueBarChart
extends JPanel {
    ...
    public MyDirectGraphMultiNodeLevelEdgeValueBarChart() {
        setMultiNodeSharedEdgeValueRankBarChart();
    }

    public void setMultiNodeSharedEdgeValueRankBarChart() {
        ...
        for (MyDirectEdge e : edges) {
            if (MyDirectGraphVars.getDirectGraphViewer().multiNodes.contains(e.getDest()) || MyDirectGraphVars.getDirectGraphViewer().multiNodes.contains(e.getSource())) {
                if (MyDirectGraphVars.directGraph.getSuccessors(e.getSource()).containsAll(MyDirectGraphVars.getDirectGraphViewer().multiNodes) ||
                    MyDirectGraphVars.directGraph.getPredecessors(e.getDest()).containsAll(MyDirectGraphVars.getDirectGraphViewer().multiNodes)) {
                    String predecessor = (e.getSource().getName().contains("x") ? MyDirectGraphSysUtil.decodeVariable(e.getSource().getName()) : MyDirectGraphSysUtil.decodeNodeName(e.getSource().getName()));
                    String successor = (e.getDest().getName().contains("x") ? MyDirectGraphSysUtil.decodeVariable(e.getDest().getName()) : MyDirectGraphSysUtil.decodeNodeName(e.getDest().getName()));
    ...
    @Override synchronized public void paint(Graphics g) {
        try {
            ...
            g.drawString("SHARED EDGE VALUES", 0, (9 * (i + 1)) + ((i * gap)));
            i++;

            int edgeCnt = 0;
            for (String name : this.data.keySet()) {
                if (edgeCnt < MyDirectGraphVars.BAR_CHART_RECORD_LIMIT && this.data.get(name) > 0) {
                    g.setColor(this.colors.get(edgeCnt++));
                    float valuePortion = this.data.get(name) / this.maxVal;
                    valuePortion = this.barWidth * valuePortion;
