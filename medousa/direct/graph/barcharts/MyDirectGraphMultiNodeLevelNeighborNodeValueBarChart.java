...
public class MyDirectGraphMultiNodeLevelNeighborNodeValueBarChart extends JPanel {
    ...
    public MyDirectGraphMultiNodeLevelNeighborNodeValueBarChart() {
    ...
    public void setNeighborNodeValueBarChart() {
        ...
        for (MyDirectEdge e : edges) {
            if (MyDirectGraphVars.getDirectGraphViewer().multiNodes.contains(e.getDest()) && MyDirectGraphVars.getDirectGraphViewer().multiNodeSharedPredecessorSet.contains(e.getSource())) {
            ...
            if (MyDirectGraphVars.getDirectGraphViewer().multiNodes.contains(e.getSource()) && MyDirectGraphVars.getDirectGraphViewer().multiNodeSharedSuccessorSet.contains(e.getDest())) {
        ...
        this.sharedPredecessors = MyDirectGraphSysUtil.sortMapByFloatValue(this.sharedPredecessors);
        this.sharedSuccessors = MyDirectGraphSysUtil.sortMapByFloatValue(this.sharedSuccessors);
    ...
    @Override public void paint(Graphics g) {
        ...
        if (this.sharedPredecessors.size() > 0 && this.sharedSuccessors.size() == 0) {
            ...
            g.drawString("SHARED PREDECESSOR VALUES", (this.getWidth()-(titleStringWidth)), (10*(i+1))+((i*gap)));
            i++;
            ...
            for (String name : this.sharedPredecessors.keySet()) {
                if (predecessorIdx < MyDirectGraphVars.BAR_CHART_RECORD_LIMIT) {
                    ...
                    String value = MyDirectGraphSysUtil.formatAverageValue(MyDirectGraphMathUtil.twoDecimalFormat(this.sharedPredecessors.get(name)));
            ...
            g.drawString("SHARED SUCCESSOR VALUES", (this.getWidth()-(titleStringWidth)), (10*(i+1))+((i*gap)));
            i++;
            ...
            for (String name : this.sharedSuccessors.keySet()) {
                if (successorIdx < MyDirectGraphVars.BAR_CHART_RECORD_LIMIT) {
                    ...
                    g.setColor(Color.BLUE);
                    g.drawRect(xPos-2, (10*i)+(i*this.gap)+1, (int)BAR_WIDTH_TO_DRAW+1, 10);
                    String value = MyDirectGraphSysUtil.formatAverageValue(MyDirectGraphMathUtil.twoDecimalFormat(this.sharedSuccessors.get(name)));
                    value = (value.endsWith(".0") || value.endsWith(".00") ? value.substring(0, value.indexOf(".0")) : value);
                    value = name + "[" + value + "]";
                    int nameWritingPos = (xPos-10) -fontMetrics.stringWidth(value);
                    g.setFont(MyDirectGraphVars.f_pln_12);
            ...
            g.setFont(MyDirectGraphVars.tahomaBoldFont12);
            g.setColor(Color.DARK_GRAY);
            g.drawString("", 5, (10*(i+1))+((i*this.gap)+1));
            i++;
