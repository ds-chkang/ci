...
public class MyDirectGraphAnalysisDirectGraphViewer
extends VisualizationViewer<MyDirectNode, MyDirectEdge>
implements Serializable {

    protected MyDirectGraphAnalysisGraphMouseListener graphMouseListener;
    ...
    public MyDirectGraphAnalysisDirectGraphViewer(VisualizationModel<MyDirectNode, MyDirectEdge> vm, MyDirectGraphAnalysisDirectGraphAnalyzer graphAnalyzer) {
        super(vm);
        try {
            ...
            this.graphMouseListener = new MyDirectGraphAnalysisGraphMouseListener(this);
            this.addGraphMouseListener(this.graphMouseListener);
            this.addMouseListener(new MyDirectGraphAnalysisDirectGraphViewerMouseListener(this, graphAnalyzer));
    ...
    public Transformer<MyDirectNode, String> defaultToolTipper = new Transformer<MyDirectNode, String>() {
        @Override public String transform(MyDirectNode n) {
                ...
                String toolTip = "<HTML>" +
                        ...
                        "NODE: " + n.getName() +
                        "<BR>" +
                        "CONTRIBUTION: " + MyDirectGraphMathUtil.getCommaSeperatedNumber(n.getContribution()) +
                        "</BODY>" +
                        "</HTML>";
                return toolTip;
    ...
    public void setEdgeContributionValue() {
        ...
        for (MyDirectEdge edge : edges) {
            ...
            for (int sequence = 0; sequence < MyDirectGraphVars.seqs.length; sequence++) {
                for (int i = 1; i < MyDirectGraphVars.seqs[sequence].length; i++) {
                    String predecessor = MyDirectGraphVars.seqs[sequence][i-1].split(":")[0];
                    String successor = MyDirectGraphVars.seqs[sequence][i].split(":")[0];
                    if (predecessor.equals(edge.getSource().getName()) && successor.equals(edge.getDest().getName())) {
                        edgeContribution++;
                    }
                }
            }
            if (((MyDirectGraphAnalysisGraph)this.getGraphLayout().getGraph()).MAX_EDGE_VALUE < edgeContribution) {
                ((MyDirectGraphAnalysisGraph)this.getGraphLayout().getGraph()).MAX_EDGE_VALUE = edgeContribution;
            }
        ...
        for (MyDirectEdge edge : edges) {
            ...
            for (int sequence = 0; sequence < MyDirectGraphVars.seqs.length; sequence++) {
                for (int i = 1; i < MyDirectGraphVars.seqs[sequence].length; i++) {
                    String predecessor = MyDirectGraphVars.seqs[sequence][i-1].split(":")[0];
                    String successor = MyDirectGraphVars.seqs[sequence][i].split(":")[0];
                    long reachTime = Long.valueOf(MyDirectGraphVars.seqs[sequence][i].split(":")[1]);
                    if (predecessor.equals(edge.getSource().getName()) && successor.equals(edge.getDest().getName())) {
                        edgeReachTime += reachTime;
                    }
                }
            }
            if (((MyDirectGraphAnalysisGraph)this.getGraphLayout().getGraph()).MAX_EDGE_VALUE < edgeReachTime) {
                ((MyDirectGraphAnalysisGraph)this.getGraphLayout().getGraph()).MAX_EDGE_VALUE = edgeReachTime;
            }
