...
public class MyDirectGraphTopLevelPredecessorCountDistribution
extends JPanel
implements ActionListener {

    protected boolean MAXIMIZED;
    private int maxPredecessor = 0;
    private int minPredecessor = 1000000000;
    private double avgPredecessor = 0;
    private double stdPredecessor = 0;

    public MyDirectGraphTopLevelPredecessorCountDistribution() {
        this.decorate();
    }
    ...
    public void enlarge() {
        ...
        try {
            MyDirectGraphTopLevelPredecessorCountDistribution topLevelPredecessorCountDistribution = new MyDirectGraphTopLevelPredecessorCountDistribution();
            topLevelPredecessorCountDistribution.MAXIMIZED = true;

            String [] statTableColumns = {"PROPERTY", "VALUE"};
            String [][] statTableData = {
                    {"MAX. PREDECESSOR", MyDirectGraphMathUtil.getCommaSeperatedNumber(maxPredecessor)},
                    {"NIN. PREDECESSOR", MyDirectGraphMathUtil.getCommaSeperatedNumber(minPredecessor)},
                    {"AVG. PREDECESSOR", MyDirectGraphSysUtil.formatAverageValue(MyDirectGraphMathUtil.twoDecimalFormat(avgPredecessor))},
                    {"STD. PREDECESSOR", MyDirectGraphSysUtil.formatAverageValue(MyDirectGraphMathUtil.twoDecimalFormat(stdPredecessor))}
            };
            ...
            String [] nodeTableColumns = {"NO.", "NODE", "P.", "P. R."};
            ...
            for (MyDirectNode n : nodes) {
                valueMap.put(n.getName(), (long) MyDirectGraphVars.directGraph.getPredecessorCount(n));
            }
            ...
            JPanel nodeTableSearchPanel = MyTableUtil.searchAndSaveDataPanelForJTable2(topLevelPredecessorCountDistribution, nodeSearchTxt, nodeSelectBtn, nodeTableModel, nodeTable);
            ...
            contentPane.setLeftComponent(topLevelPredecessorCountDistribution);
    ...
    private JFreeChart setChart() {
        ...
        Collection<MyDirectNode> nodes = MyDirectGraphVars.directGraph.getVertices();
        for (MyDirectNode n : nodes) {
            int value = MyDirectGraphVars.directGraph.getPredecessorCount(n);
            totalPredecessors += value;
            nodeCount++;

            if (value > this.maxPredecessor) {
                this.maxPredecessor = value;
            }

            if (value > 0 && value < this.minPredecessor) {
                this.minPredecessor = value;
            }
        ...
        this.avgPredecessor = (double) totalPredecessors/nodeCount;
        this.stdPredecessor = getPredecessorCountStandardDeviation(valueMap);

        DefaultCategoryDataset dataset = new DefaultCategoryDataset();
        for (Integer avgPredecessorCount : valueMap.keySet()) {
    ...
    public double getPredecessorCountStandardDeviation(TreeMap<Integer, Integer> valueMap) {
        double sum = 0.00d;
