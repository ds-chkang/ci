...
extends MyNotifyingPrefixer {
    ...
    throws IOException {
        if (prefix.size() > MySequentialGraphVars.minSub) {
            this.bw.write(prefix.pattern + MySequentialGraphVars.contributionSymbol + prefix.size());
        } else {
    ...
    private BufferedWriter createFileWriter(MyPrefix prefix) {
        try {
            ...
            String fileNameSuffix = prefix.getUniqueContribution() > MySequentialGraphVars.minSub ?
                    (MySequentialGraphVars.contributionSymbol + prefix.getUniqueContribution() + "!" + prefix.totalContribution) :
                    ("!" + prefix.totalContribution);
            this.patternFile = new File(this.outputDir + MySequentialGraphSysUtil.getDirectorySlash() + prefix.pattern + fileNameSuffix);
    ...
    private void closeFileWriter() {
        try {
            File renameFile = new File(this.outputDir + MySequentialGraphSysUtil.getDirectorySlash() + this.patternFile.getName()+ MySequentialGraphVars.patternCountSymbol+this.cnt);
            bw.close();
