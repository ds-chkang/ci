...
public class MyDepthLevelStartingNodeValueHistogramDistributionLineChart
extends JPanel
implements ActionListener {
    ...
    public MyDepthLevelStartingNodeValueHistogramDistributionLineChart() {
        this.decorate();
    }
                        ...
                        for (String depthNode : MySequentialGraphVars.getSequentialGraphViewer().vc.depthNodeNameSet) {
                            for (int s = 0; s < MySequentialGraphVars.seqs.length; s++) {
                                for (int i = 1; i < MySequentialGraphVars.seqs[s].length; i++) {
                                    ...
                                    if (n.equals(depthNode)) {
                                        String startingNode = MySequentialGraphVars.seqs[s][0].split(":")[0];
                                        startingNode = (startingNode.contains("x") ? MySequentialGraphSysUtil.decodeVariable(startingNode) : MySequentialGraphSysUtil.getDecodedNodeName(startingNode));
