...
public class MyGraphLevelNodeUniqueContributionDistributionLineChart
extends JPanel {
    ...
    public MyGraphLevelNodeUniqueContributionDistributionLineChart() {
    ...
    private void setNodeUniqueContributionBarChart() {
        ...
        JLabel titleLabel = new JLabel(" N. U. C.");
        titleLabel.setToolTipText("NODE UNIQUE CONTRIBUTION DISTRIBUTION");
        ...
        SwingUtilities.invokeLater(new Runnable() {
            @Override public void run() {
                try {
                    ...
                    JLabel titleLabel = new JLabel(" N. U. C.");
                    titleLabel.setToolTipText("NODE UNIQUE CONTRIBUTION VALUE DISTRIBUTION");
                    titleLabel.setFont(MySequentialGraphVars.tahomaBoldFont11);
                    titleLabel.setBackground(Color.WHITE);
                    titleLabel.setForeground(Color.DARK_GRAY);
                    ...
                    chartMenu.addActionListener(new ActionListener() {
                        @Override public void actionPerformed(ActionEvent e) {
                            if (chartMenu.getSelectedIndex() == 0) {
                                SwingUtilities.invokeLater(new Runnable() {
                                    @Override public void run() {setNodeUniqueContributionBarChart();}
                                });
                            } else if (chartMenu.getSelectedIndex() == 1) {
                                SwingUtilities.invokeLater(new Runnable() {
                                    @Override public void run() {
    ...
    private JFreeChart setNodeChart() {
        ...
        String plotTitle = "";
        String xaxis = "NODE UNIQUE CONTRIBUTION[AVG.: " + MySequentialGraphSysUtil.formatAverageValue(MyMathUtil.twoDecimalFormat(avgUniqCont)) + "]";
        String yaxis = "";
        PlotOrientation orientation = PlotOrientation.VERTICAL;
        boolean show = false;
        boolean toolTips = true;
        boolean urls = false;
        return ChartFactory.createXYLineChart("", "N. U. C.", "", dataset);
    }
    ...
    public void enlarge() {
        ...
        try {
            MAXIMIZED = true;
            JFrame f = new JFrame(" NODE UNIQUE CONTRIBUTION DISTRIBUTION");
            f.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
            f.setLayout(new BorderLayout(3, 3));
            f.getContentPane().add(new MyGraphLevelNodeUniqueContributionDistributionLineChart(), BorderLayout.CENTER);
