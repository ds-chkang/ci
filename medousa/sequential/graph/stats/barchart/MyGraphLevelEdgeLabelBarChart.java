...
public class MyGraphLevelEdgeLabelBarChart
extends JPanel {
    ...
    public MyGraphLevelEdgeLabelBarChart() {}

    public void setEdgeLabelBarChart() {
        this.data = new LinkedHashMap<>();
        this.colors = new ArrayList<>();
        Collection<MyEdge> edges = MySequentialGraphVars.g.getEdges();
        for (MyEdge e : edges) {
            if (e.getDest().getCurrentValue() == 0 || e.getSource().getCurrentValue() == 0) continue;
            if (e.edgeLabelMap.containsKey(MySequentialGraphVars.getSequentialGraphViewer().vc.edgeLabelSelecter.getSelectedItem().toString())) {
                String edgeLabelValue = e.edgeLabelMap.get(MySequentialGraphVars.getSequentialGraphViewer().vc.edgeLabelSelecter.getSelectedItem().toString());
                if (this.data.containsKey(e.edgeLabelMap.get(edgeLabelValue))) {
                    this.data.put(edgeLabelValue, this.data.get(edgeLabelValue) + 1);
                } else {
                    this.data.put(edgeLabelValue, 1L);
    ...
    @Override public void paint(Graphics g) {
        try {
            ...
            g.drawString("EDGE LABLES", 0, (10 * (i + 1)) + ((i * gap)));
            i++;
