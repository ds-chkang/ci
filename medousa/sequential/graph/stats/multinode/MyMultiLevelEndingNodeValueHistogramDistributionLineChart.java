...
implements ActionListener {
                    ...
                    for (MyNode selectedNode : MySequentialGraphVars.getSequentialGraphViewer().multiNodes) {
                        for (int s = 0; s < MySequentialGraphVars.seqs.length; s++) {
                            for (int i = 0; i < MySequentialGraphVars.seqs[s].length; i++) {
                                ...
                                if ((i + 1) != MySequentialGraphVars.seqs[s].length && n.equals(selectedNode.getName())) {
                            ...
                            if (!MAXIMIZED && colors.size() == 6) {break;}
                            else if (MAXIMIZED && colors.size() == 200) {break;}
                        }
                        if (!MAXIMIZED && colors.size() == 6) {break;}
                        else if (MAXIMIZED && colors.size() == 50) {break;}
                    }
    ...
    public void enlarge() {
        ...
        try {
            ...
            f.getContentPane().add(new MyMultiLevelEndingNodeValueHistogramDistributionLineChart(), BorderLayout.CENTER);
