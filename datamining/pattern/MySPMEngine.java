...
implements MyPrefixerCompleteListener {
    ...
    public MySPMEngine() {
        try {
            ...
            outputDir = MySysUtil.getWorkingDir() + MySysUtil.getDirectorySlash() + "patterns";
    ...
    public MySPMEngine(ArrayList<String> data) {
        try {
            this.pb = new MyProgressBar(false);
            this.outputDir = MySysUtil.getWorkingDir() + MySysUtil.getDirectorySlash() + "patterns";
            this.pb.setAlwaysOnTop(false);
            MyVars.globalPatternCount = 0L;
            this.availableProcessors = Runtime.getRuntime().availableProcessors();
    ...
    public void printStatistics() {
        ...
        r.append(MyMathUtil.getCommaSeperatedNumber(MyVars.globalPatternCount));
        r.append('\n');
    ...
    throws IOException {
        ...
        if (MyVars.isTimeOn) {
            while ((line = br.readLine()) != null) {
                String[] iSets = line.trim().split(MyVars.hyphenDelimeter);
                seqs[s] = new String[iSets.length][];
                for (int iSet = 0; iSet < seqs[s].length; iSet++) {
                    seqs[s][iSet] = iSets[iSet].split(MyVars.commaDelimeter);
                ...
                if (iSets.length > MyVars.mxDepth) {
                    MyVars.mxDepth = iSets.length;
                }
                s--;
            }
        } else {
