...
public class MyMultiNodeLevelEdgeValueBarChart
extends JPanel {
    ...
    public MyMultiNodeLevelEdgeValueBarChart() {
        setMultiNodeSharedEdgeValueRankBarChart();
    }

    public synchronized void setMultiNodeSharedEdgeValueRankBarChart() {
        ...
        for (MyDirectEdge e : edges) {
            if (MyVars.getDirectGraphViewer().multiNodes.contains(e.getDest()) || MyVars.getDirectGraphViewer().multiNodes.contains(e.getSource())) {
                if (MyVars.directMarkovChain.getSuccessors(e.getSource()).containsAll(MyVars.getDirectGraphViewer().multiNodes) ||
                    MyVars.directMarkovChain.getPredecessors(e.getDest()).containsAll(MyVars.getDirectGraphViewer().multiNodes)) {
    ...
    @Override synchronized public void paint(Graphics g) {
        try {
            ...
            for (String name : this.data.keySet()) {
                if (edgeCnt < MyVars.BAR_CHART_RECORD_LIMIT && this.data.get(name) > 0) {
