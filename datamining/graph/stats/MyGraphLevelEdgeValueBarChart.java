...
extends JPanel {
    ...
    public void setEdgeValueBarChartForDepthNodes() {
        ...
        if (MyVars.getViewer().vc.depthNeighborNodeTypeSelector.getSelectedItem().toString().contains("S.")) {
            Map<String, Map<String, Integer>> depthNodeSuccessorMaps = MyVars.getViewer().vc.depthNodeSuccessorMaps;
            for (MyEdge e : edges) {
                if (depthNodeSuccessorMaps.keySet().contains(e.getSource().getName())) {
                    for (String depthNode : depthNodeSuccessorMaps.keySet()) {
                        ...
                        if (successorSet.containsKey(e.getDest().getName())) {
                            String predecessor = (e.getSource().getName().contains("x") ? MySysUtil.decodeVariable(e.getSource().getName()) : MySysUtil.decodeNodeName(e.getSource().getName()));
                            String successor = (e.getDest().getName().contains("x") ? MySysUtil.decodeVariable(e.getDest().getName()) : MySysUtil.decodeNodeName(e.getDest().getName()));
                            String name = predecessor + "-" + successor;
                            this.data.put(name, (float) e.getDest().getNodeDepthInfo(MyVars.currentGraphDepth +1).getInContribution());
                            e.setCurrentValue((float) e.getDest().getNodeDepthInfo(MyVars.currentGraphDepth +1).getInContribution());
            ...
            Map<String, Map<String, Integer>> depthNodePredecessorSetMap = MyVars.getViewer().vc.depthNodePredecessorMaps;
            for (MyEdge e : edges) {
                if (depthNodePredecessorSetMap.keySet().contains(e.getDest().getName())) {
                    for (String depthNode : depthNodePredecessorSetMap.keySet()) {
                        ...
                        if (predecessorMap.containsKey(e.getSource().getName())) {
                            ...
                            String name = predecessor + "-" + successor;
                            this.data.put(name, (float) e.getDest().getNodeDepthInfo(MyVars.currentGraphDepth -1).getInContribution());
                            e.setCurrentValue((float) e.getSource().getNodeDepthInfo(MyVars.currentGraphDepth -1).getInContribution());
                            if (e.getCurrentValue() > edgeMaxValue) {
                                edgeMaxValue = e.getCurrentValue();
                            }
                            final float hue = this.rand.nextFloat();
        ...
        MyVars.g.MX_E_VAL = edgeMaxValue;
    ...
    public void setEdgeValueBarChart() {
        ...
        for (MyEdge e : edges) {
            if (MyVars.getViewer().edgeValName.contains("NONE")) {continue;}
    ...
    public void setEdgeValueBarChartForSelectedNode() {
        ...
        Collection<MyEdge> edges = MyVars.g.getEdges();
        for (MyEdge e : edges) {
            if (e.getSource() != MyVars.getViewer().selectedNode && e.getDest() != MyVars.getViewer().selectedNode) continue;
            else if (e.getDest().getCurrentValue() ==0 || e.getSource().getCurrentValue() == 0) continue;
            String predecessor = (e.getSource().getName().contains("x") ? MySysUtil.decodeVariable(e.getSource().getName()) : MySysUtil.decodeNodeName(e.getSource().getName()));
            String successor = (e.getDest().getName().contains("x") ? MySysUtil.decodeVariable(e.getDest().getName()) : MySysUtil.decodeNodeName(e.getDest().getName()));
            String name = predecessor+"-"+successor;
    ...
    @Override public void paint(Graphics g) {
        try {
            if (MyVars.getViewer().edgeValName.contains("NONE")) {return;}
