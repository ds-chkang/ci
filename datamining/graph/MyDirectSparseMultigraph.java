...
public class MyDirectSparseMultigraph<V, E>
extends AbstractTypedGraph<V, E> {

    protected Map<V, Pair<Set<E>>> vertices = new HashMap();
    protected Map<E, Pair<V>> edges = new HashMap();

    public Map<String, MyDirectNode> vRefs = new HashMap<>();
    public Set<String> edRefs =  new HashSet();;

    public MyDirectSparseMultigraph(EdgeType edgeType) { super(edgeType); }
    public MyDirectSparseMultigraph() {
    ...
    public boolean isEdgeExists(String edgeName) { return this.edRefs.contains(edgeName); }
    public boolean isVertexExists(String node) { return this.vRefs.containsKey(node); }
    public MyDirectNode getVertex(String nodeName) { return this.vRefs.get(nodeName); }
    ...
    public void addEdge(E edge, Pair<? extends V> endpoints, EdgeType edgeType) {
        //this.validateEdgeType(edgeType);
        //Pair<V> new_endpoints = this.getValidatedEndpoints(edge, endpoints);
        //if (new_endpoints == null) { return false; }
        //else {
            this.edges.put(edge, new Pair(endpoints.getFirst(), endpoints.getSecond()));
            //V source = new_endpoints.getFirst();
            //V dest = new_endpoints.getSecond();
            this.getIncoming_internal(endpoints.getSecond()).add(edge);
            this.getOutgoing_internal(endpoints.getFirst()).add(edge);
           // return true;
        //}
        //return true;
    }
