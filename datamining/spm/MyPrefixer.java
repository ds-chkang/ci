...
extends MyNotifyingPrefixer
{
    ...
    public MyPrefixer() {}
    public MyPrefixer(MyPrefix prior) {
    ...
    throws IOException {
        ...
        char length = prior.pattern.charAt(0); length++;
    ...
    private BufferedWriter createFileWriter(MyPrefix prefix) {
        try {
            File dirFile = new File(MyVars.patternOutputDir);
            if (!dirFile.exists()) {
                dirFile.mkdir();
            }
            String fileNameSuffix = prefix.getUniqueContribution() > MyVars.minSup ?
                    (MyVars.contributionSymbol + prefix.getUniqueContribution() + "!" + prefix.totalContribution) :
                    ("!" + prefix.totalContribution);
            this.patternFile = new File(MyVars.patternOutputDir + prefix.pattern + fileNameSuffix);
    ...
    private void closeFileWriter() {
        try {
            File renameFile = new File(MyVars.patternOutputDir+this.patternFile.getName()+MyVars.patternCountSymbol+this.cnt);
            bw.close();
